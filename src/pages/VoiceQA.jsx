import { StudyMaterial, StudySession } from '@/api/entities';
...
export default function VoiceQA() {
  ...
  const { data: materials = [] } = useQuery({
    queryKey: ["materials"],
    queryFn: () => StudyMaterial.filter({}, "-created_at", 200),
  });
  ...
  return (
    <div>
      ...
      {!material ? (
        ...
        <EmptyState
          icon={MessageSquare}
          title="This material has no content"
          description="The selected material doesn't have any readable content yet. Try uploading another one."
        />
      ) : (
        <div className="max-w-3xl mx-auto bg-card border border-border/60 rounded-3xl p-6 deckle-edge">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <Button onClick={send} disabled={!input.trim() || thinking || !hasContent} className="rounded-full shrink-0">
